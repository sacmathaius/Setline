(() => {
  ...
  function OnboardingDialog({open,data,update,onClose}){
    ...
    const autoPlan=generatePlan(draft.split||'push_pull_legs',trainingDays);
    useEffect(()=>{if(!planTouched)setManualPlan(autoPlan);},[draft.split,trainingDays,planTouched]);
    const previewPlan=manualPlan||autoPlan;
    ...
    const movePlanDay=(index,direction)=>{const target=index+direction;if(target<0||target>6)return;setPlanTouched(true);setManualPlan(current=>{const base=[...(current||autoPlan)];const temp=base[index];base[index]=base[target];base[target]=temp;return base;});};
    const resetPlanOrder=()=>{setPlanTouched(false);setManualPlan(autoPlan);};
    const finish=()=>{const safeDays=clamp(draft.trainingDays??4,1,7);const finalPlan=manualPlan||generatePlan(draft.split,safeDays);update(next=>{const equipment=Array.isArray(draft.equipment)?draft.equipment:[];const avoidMovements=Array.isArray(draft.avoidMovements)?draft.avoidMovements:[];next.profile={...next.profile,...draft,trainingDays:safeDays,equipment,avoidMovements,avoid:avoidMovements.join(', '),avoidNote:draft.avoidNote||''};next.settings.theme=themeMode;next.preferredUnit=normalizeUnit(unitMode);next.weeklyPlan=finalPlan;next.scheduleMeta.configured=true;next.onboardingComplete=true;});onClose();};
